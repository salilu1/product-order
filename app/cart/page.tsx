...
export default function CartPage() {
  ...
  // Checkout or retry payment
  const handleCheckout = async (orderId?: string) => {
    if (cartItems.length === 0) return;
    setLoading(true);

    try {
      // 1️⃣ Create order if no orderId (new checkout)
      let orderData: any;
      if (!orderId) {
        ...
        orderData = await safeJson(orderRes);
        if (!orderRes.ok) throw new Error(orderData?.error || "Order creation failed");
      } else {
        // Retry payment: pass existing orderId
        orderData = { id: orderId };
      }

      // 2️⃣ Initialize payment with Chapa
      ...
      if (!payData?.checkoutUrl) throw new Error("Missing checkout URL from Chapa");

      // 3️⃣ Redirect to Chapa checkout
